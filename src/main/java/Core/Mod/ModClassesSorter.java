...
class ModClassesSorter {
    ...
    private static final Collection<Pair<Pair<Class<? extends ModImplement>, Pair<String, Pair<String, HVersionComplex>>>, Pair<Pair<Set<Pair<Boolean, Pair<String, HVersionComplex>>>, Set<Pair<Boolean, Pair<String, HVersionComplex>>>>, Pair<Boolean, Boolean>>>> modContainer = new HashSet<>();
    ...
    static void buildModContainer() {/*
        for (Class<? extends ModImplement> modClass: ModManager.getModList()) {
            ...
            if (mod == null)
                continue;
            String modName = HStringHelper.notNullOrEmpty(mod.name().strip());
            String modVersion = HStringHelper.notNullOrEmpty(mod.version().strip());
            String modAvailable = HStringHelper.notNullOrEmpty(mod.availableCraftworldVersion().strip());
            HVersionComplex availableCraftworldVersion = new HVersionComplex(modAvailable);
            ...
            String[] modRequirements = HStringHelper.notNullOrEmpty(HStringHelper.strip(mod.require().split(";")));
            for (String modRequirement: modRequirements) {
                ...
                if (locationAt == -1 || locationAt == requirementInformation.length() - 1) {
                    ...
                    versionRange = new HVersionComplex();
                }
                else {
                    ...
                    versionRange = new HVersionComplex(requirementVersion);
                }
                switch (requirementModification) {
    ...
    static void checkModContainer() {/*
        for (Pair<Pair<Class<? extends ModImplement>, Pair<String, Pair<String, HVersionComplex>>>,
